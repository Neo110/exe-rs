...
impl<'data> ResourceDirectory<'data> {
    ...
    pub fn get_first_icon_group<P: PE>(&self, pe: &'data P) -> Result<Option<(ResolvedDirectoryID, GrpIconDir)>, Error> {
        let groups = self.filter(Some(ResolvedDirectoryID::ID(ResourceID::GroupIcon as u32)), None, None);
    
        if let Some(first_group) = groups.into_iter().next() {
            let id = first_group.rsrc_id.clone();
            let entry = first_group.get_data_entry(pe)?;
            let dir = GrpIconDir::parse(pe, entry.offset_to_data)?;
